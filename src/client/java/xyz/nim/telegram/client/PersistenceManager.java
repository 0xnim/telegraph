...
public class PersistenceManager {
    private static final String CONFIG_DIR = "config/telegram";
    ...
    public void saveChannelSettings(TelegramChannel channel) {
    ...
    public void loadChannelSettings(TelegramChannel channel) {
        Path configDir = getConfigDir();
        if (configDir == null) return;
    ...
    public void saveMessages(TelegramChannel channel) {
        Path configDir = getConfigDir();
        if (configDir == null) return;
        ...
        try {
            ...
            for (int mapId : channel.getAllChannelIds()) {
                List<TelegramMessage> messages = channel.getMessages(mapId);
    ...
    public void exportChannelData(int mapId, TelegramChannel channel, Path exportPath) {
        try {
