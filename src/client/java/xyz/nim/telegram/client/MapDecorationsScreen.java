...
public class MapDecorationsScreen extends Screen {
    ...
    public MapDecorationsScreen(TelegramChannel channel) {
    ...
    protected void init() {
        ...
        ButtonWidget composeButton = ButtonWidget.builder(Text.literal("Compose"), button -> {
            if (selectedMapId != -1 && client != null) {
                ...
                if (settings != null && settings.getProtocol() instanceof xyz.nim.telegram.client.protocol.CarniteProtocol) {
                    client.setScreen(new xyz.nim.telegram.client.carnite.CarniteComposerScreen(this, channel, selectedMapId, settings));
                } else {
    ...
    private void cycleProtocol() {
        ...
        if (settings.getProtocol() instanceof xyz.nim.telegram.client.protocol.MapTelegraphProtocol) {
            settings.setProtocol(new xyz.nim.telegram.client.protocol.CarniteProtocol());
        } else {
            settings.setProtocol(new xyz.nim.telegram.client.protocol.MapTelegraphProtocol());
        }
        updateProtocolButtons();
        updateMessageList();
    }
    ...
    private void updateProtocolButtons() {
        ...
        boolean isTelegraphProtocol = settings.getProtocol() instanceof xyz.nim.telegram.client.protocol.MapTelegraphProtocol;
    ...
    public void render(DrawContext context, int mouseX, int mouseY, float delta) {
        ...
        if (selectedMapId != -1) {
            ...
            boolean isTelegraphProtocol = settings != null && 
                settings.getProtocol() instanceof xyz.nim.telegram.client.protocol.MapTelegraphProtocol;
    ...
    private void updateMessageList() {
        ...
        List<TelegramMessage> messages = new ArrayList<>(channel.getMessages(selectedMapId));
        ...
        if (currentTab == ViewTab.RAW) {
            ChannelSettings settings = channel.getSettings(selectedMapId);
            boolean isCarnite = settings != null && settings.getProtocol() instanceof xyz.nim.telegram.client.protocol.CarniteProtocol;
            ...
            for (TelegramMessage message : messages) {
                ...
                if (isCarnite && message.decoration() != null && message.decoration().name() != null) {
                    String tense = xyz.nim.telegram.client.carnite.CarniteParser.getTenseFromColor(message.decoration().type());
                    String expanded = xyz.nim.telegram.client.carnite.CarniteVocabulary.formatWithExpansion(message.decoration().name());
                    tooltip = "§e" + tense + "\n§7" + expanded;
                }
            ...
            if (formattedMessages.isEmpty()) {
                ...
                for (FormattedMapMessage msg : formattedMessages) {
                    ...
                    String tooltip = null;
                    if (isCarnite) {
                        xyz.nim.telegram.client.carnite.CarniteParser.ParsedCarniteMessage parsed = 
                            xyz.nim.telegram.client.carnite.CarniteParser.parse(msg.name, msg.bannerType);
                        String tense = xyz.nim.telegram.client.carnite.CarniteParser.getTenseFromColor(msg.bannerType);
                        String expanded = xyz.nim.telegram.client.carnite.CarniteVocabulary.formatWithExpansion(msg.name);
                        tooltip = "§e" + tense + "\n§7" + expanded;
                    }
    ...
    private List<FormattedMapMessage> extractMapMessages(List<TelegramMessage> messages) {
        ...
        for (TelegramMessage message : messages) {
            if (message.decoration() != null && message.decoration().type().contains("banner")) {
                if (message.type() == TelegramMessage.ChangeType.ADDED && message.decoration().name() != null) {
    ...
    private int getColorForType(TelegramMessage.ChangeType type) {
    ...
    private String getTypePrefix(TelegramMessage.ChangeType type) {
        if (type == null) return "";
        return switch (type) {
            case ADDED -> "[+] ";
            case REMOVED -> "[-] ";
